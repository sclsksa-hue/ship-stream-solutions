...
import { Search, UserPlus, Building2, Target, Ship, FileText } from "lucide-react";
...
export default function GlobalSearch() {
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(async () => {
      const q = `%${query}%`;
      const [leads, custs, opps, ships, quotes] = await Promise.all([
        ...
        supabase.from("shipments").select("id, shipment_number, origin, destination").ilike("shipment_number", q).limit(5),
      ...
      const r: Result[] = [
        ...
        ...(ships.data || []).map(s => ({ type: "shipment", id: s.id, label: s.shipment_number, sub: `${s.origin || ""} → ${s.destination || ""}` })),
  ...
  const icon = (type: string) => {
    ...
    if (type === "shipment") return <Ship className={cls} />;
    return <FileText className={cls} />;
  };

  const go = (r: Result) => {
    const routes: Record<string, string> = { lead: "/leads", customer: "/customers", opportunity: "/opportunities", shipment: "/shipments", quotation: "/quotations" };
  ...
  return (
    <div ref={ref} className="relative w-full max-w-md">
      <div className="relative">
        ...
        <Input
          placeholder="Search leads, customers, shipments..."
          value={query}
        ...
        <div className="absolute top-full left-0 right-0 mt-1 rounded-lg border bg-popover shadow-lg z-50 max-h-80 overflow-y-auto">
          {results.map((r, i) => (
            <button
              key={`${r.type}-${r.id}-${i}`}
              onClick={() => go(r)}
              className="w-full flex items-center gap-3 px-4 py-2.5 text-sm hover:bg-accent/10 transition-colors text-left"
            >
              {icon(r.type)}
              ...
              <span className="text-[10px] uppercase font-medium text-muted-foreground bg-muted px-1.5 py-0.5 rounded">{r.type}</span>
            </button>
          ))}
        </div>
      )}
